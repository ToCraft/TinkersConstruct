...
import static slimeknights.tconstruct.util.ColourUtils.combine;
import static slimeknights.tconstruct.util.ColourUtils.getA;
import static slimeknights.tconstruct.util.ColourUtils.getB;
import static slimeknights.tconstruct.util.ColourUtils.getG;
import static slimeknights.tconstruct.util.ColourUtils.getR;
  ...
  protected static class Deserializer implements JsonDeserializer<GreyToColorMapping> {
    ...
    public GreyToColorMapping deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
      ...
      for (int i = 0; i < palette.size(); i++) {
        ...
        String colorString = GsonHelper.getAsString(palettePair, "color");
        int color = !colorString.isEmpty() ? ColorLoadable.ALPHA.parseString(colorString, "[unknown]") : -1;
        if (i == 0 && grey != 0) {
